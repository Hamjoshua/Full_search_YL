...
map_params = {
    "l": "map",
    "pt": f'{original_coordinates},pm2rdl'
}
...
for org in organizations:
    if count <= 10:
        ...
        if toponym_of_org.get('Hours', False):
            if toponym_of_org['Hours']['Availabilities'][0].get('TwentyFourHours', False):
                mark = ',pm2gnm'
            else:
                mark = ',pm2blm'
        else:
            mark = ',pm2grm'
        map_params['pt'] = map_params['pt'] + f"~{drug_store_coords}{mark}"
    else:
        break
...
with Image.open(BytesIO(response.content)) as im:
    ...
    try:
        drawer.text((10, y_margin), 'Топ 10 аптек до вашего адреса', font=font)
    except NameError:
